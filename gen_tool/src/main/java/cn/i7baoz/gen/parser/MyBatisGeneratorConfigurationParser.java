...
import cn.i7baoz.gen.factory.MyContext;
...
public class MyBatisGeneratorConfigurationParser {
    ...
    public MyConfiguration parseConfiguration(Element rootNode)
    ...
    private void parseProperties(MyConfiguration configuration, Node node)
            throws XMLParserException {
    ...
    private void parseContext(MyConfiguration configuration, Node node) {
        ...
        MyContext context = new MyContext(mt);
        context.setId(id);
    ...
    private void parseSqlMapGenerator(MyContext context, Node node) {
    ...
    private void parseTable(MyContext context, Node node) {
        MyTableConfiguration tc = new MyTableConfiguration(context);
        ...
        //批量新增
        String enableBatchInsert = attributes.getProperty("enableBatchInsert");
        //批量更新
        ...
        if (stringHasValue(enableBatchInsert)) {
            tc.setBatchInsertStatementEnabled(isTrue(enableBatchInsert));
        }
    ...
    private void parseColumnOverride(MyTableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
    ...
    private void parseGeneratedKey(MyTableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
    ...
    private void parseIgnoreColumn(MyTableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
        String column = attributes.getProperty("column"); //$NON-NLS-1$
        String delimitedColumnName = attributes
                .getProperty("delimitedColumnName"); //$NON-NLS-1$
    ...
    private void parseColumnRenamingRule(MyTableConfiguration tc, Node node) {
        Properties attributes = parseAttributes(node);
    ...
    private void parseJavaTypeResolver(MyContext context, Node node) {
    ...
    private void parsePlugin(MyContext context, Node node) {
    ...
    private void parseJavaModelGenerator(MyContext context, Node node) {
    ...
    private void parseJavaClientGenerator(MyContext context, Node node) {
    ...
    private void parseJdbcConnection(MyContext context, Node node) {
    ...
    private void parseClassPathEntry(MyConfiguration configuration, Node node) {
        Properties attributes = parseAttributes(node);
    ...
    private void parseCommentGenerator(MyContext context, Node node) {
